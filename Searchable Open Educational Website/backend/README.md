...
```bash
my-application$ sam local invoke putItemFunction --event json/event-post-item.json
my-application$ sam local invoke getAllItemsFunction --event json/event-get-all-items.json
```
...
```bash
aws cloudformation delete-stack --stack-name oerinspanish
```
