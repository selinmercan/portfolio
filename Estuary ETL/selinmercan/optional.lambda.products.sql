...
-- Example statement which passes-through source selinmercan/takehome/products documents without modification.
-- Use a WHERE clause to filter, for example: WHERE $my$column = 1234
SELECT JSON($flow_document);

-- Example statement demonstrating how to SELECT specific locations from documents of selinmercan/takehome/products.
...
SELECT
    ...
    $_meta$before,
    -- Field _meta/before/price at /_meta/before/price; (source type: int4)
    $_meta$before$price,
    -- Field _meta/before/product_id at /_meta/before/product_id; (source type: non-nullable int4)
    $_meta$before$product_id,
    -- Field _meta/before/product_name at /_meta/before/product_name; (source type: text)
    $_meta$before$product_name,
    ...
    $flow_published_at,
    -- Field price at /price; (source type: int4)
    $price,
    -- Key product_id at /product_id; (source type: non-nullable int4)
    $product_id,
    -- Field product_name at /product_name; (source type: text)
    $product_name
