...
#include <cstdlib>
using namespace std;
bool total_returns_true(map<int, int>& variables, vector<vector<int> >& clause);
bool total_returns_false(map<int, int>& variables, vector<vector<int> >& clause);
bool clause_returns_true(map<int, int>& variables, vector<int>& clause);
bool solver(map<int, int>& variables, vector<vector<int> >& clause, int num_variables, int current_variable, ofstream& os, int& final_value){
	...
	if(total_returns_true(variables, clause)){//if the whole expression returns true, print the variables
		for(map<int, int>::iterator it=variables.begin(); it!=variables.end(); ++it){
			os<<it->first<<" = "<<it->second<<endl;
		} 
		return true;
	}
	for(int i=1; i>=0; i--){
		variables[current_variable]=i;
...
bool total_returns_true(map<int, int>& variables, vector<vector<int> >& clause){//this function returns true if the whole expression is true. if there are clauses with all of their  
